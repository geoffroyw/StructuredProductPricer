...
//Calcul des Grecques par diff�rence divis�es
...
void BonusCliquet::setSpotPrice(double price) {
	if(price<=0) {
		throw out_of_range("Le prix Spot doit �tre strictement positif");
	}
	else {
		spotPrice = price;
	}
...
void BonusCliquet::setLowBarrier(double b) {
	if(b<=0) {
		throw out_of_range("La barri�re inf�rieure doit �tre strictement positive");
	}
	else {
		lowBarrier=b;
	}
...
void BonusCliquet::setHighBarrier(double b) {
	if(b<=0) {
		throw out_of_range("La barri�re sup�rieure doit �tre strictement positive");
	}
	else {
		highBarrier = b;
	}
...
void BonusCliquet::setVol(double v) {
	if(v<0 || v>1) {
		throw out_of_range("La volatilit� doit �tre comprise entre 0 et 1");
	}
	else {
		vol = v;
	}
